...
The primary user flow is search-first: a customer searches for a item or service, Ask shows matching catalog/service results when the data exists, and the customer immediately understands which business can help. If exact item/service data is missing or the customer wants confirmation, Ask can send a fallback request to suitable sellers or service providers.

The item is broader than request routing. Ask should become a reliable local item and service search layer between customers and stores or service providers. That requires business onboarding, item/service catalogs, Excel/CSV import, data normalization, Smart Search, search indexing, clear response rules, provider outreach, and integration boundaries for systems that suppliers already use.

## The Problem

Customers often need something now but do not know where it is actually available. They may search several marketplaces, call shops, write to messengers, compare incomplete listings, and still discover that the item is unavailable or the service has no free time.
...
A classic marketplace usually starts with a tightly controlled catalog. Ask starts from local item/service search:
...
Ask should not force every supplier into perfect catalog migration before the item is useful. It should still build toward catalog/search as the core item and keep manual requests as a fallback for missing, stale, or uncertain data.
...
6. Search results that show where a known item or service is available when data supports it.
...
Avoid a structure that grows into broad technical buckets such as one global `controller`, `service`, `repository`, `dto`, `mapper`, and `entity` package for every item area. That shape becomes hard to navigate as Ask grows across customers, sellers, requests, responses, categories, branches, notifications, moderation, integrations, analytics, catalog import, and services.
...
Product sellers usually already have item data in files or systems. The backend must support practical import paths, especially Excel and CSV, so sellers do not have to manually recreate catalogs from scratch. Catalog work should include file upload, column mapping, validation, normalization, duplicate handling, category/attribute mapping, branch-level data, import history, and data-quality feedback.

Catalog is now part of the core search strategy, not a distant optional add-on. If a item or service already exists in Ask data, the backend should be able to return it through search before creating a manual request.
...
Current item surface direction:
...
- item-catalog bulk import must support Excel and CSV workflows;
- customer search should show known products/services first and use requests only when data is insufficient;
- future web surfaces may also support bulk item catalog operations if that becomes the most usable supplier workflow, but this should remain API-backed and not become a separate backend.

## Current Foundation Scope

This repository is a foundation for future Ask backend and AI-assisted development. It preserves the item vision, backend code rules, data architecture, client contract expectations, and first-session Codex setup notes that new developers and Codex agents should load before coding.

It is not an old prototype dump and not a local-machine-specific Codex export.

## Important Documents

- `AGENTS.md`: short agent workflow, first-session pointer, task routing, and item guardrails.
...
- `AI_Knowledge/data_architecture/PRODUCT_SERVICE_FOUNDATION_ERD.md`: MVP entity strategy, ERD, table connections, booking, messaging, and item/service search boundaries.
