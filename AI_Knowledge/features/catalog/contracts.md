...
| POST | /api/v1/business-admin/branches/{branchId}/products | OWNER/MANAGER/WORKER | Create item + offer |
| PATCH | /api/v1/business-admin/branches/{branchId}/products/{productId} | OWNER/MANAGER/WORKER | Update item |
| DELETE | /api/v1/business-admin/branches/{branchId}/products/{productId} | OWNER/MANAGER/WORKER | Delete/disable item |

## Product Offer Model
- Product (business-owned SKU): name, description, one category_id, tags, sku, attributes (JSONB), status
- ProductOffer (branch-level): product_id, branch_id, price, enabled, status
- ProductOffer is the only branch relationship; duplicating to another branch creates a new offer with copied initial price.
...
| Method | Path | Auth | Purpose |
|--------|------|------|---------|
| POST | /api/v1/business-admin/branches/{branchId}/item-imports | OWNER/WORKER/PLATFORM | Upload .xlsx |
| POST | /api/v1/business-admin/branches/{branchId}/item-imports/{importId}/mapping | OWNER/WORKER/PLATFORM | Save column mapping |
| GET | /api/v1/business-admin/branches/{branchId}/item-imports/{importId}/preview | OWNER/WORKER/PLATFORM | Get preview |
| POST | /api/v1/business-admin/branches/{branchId}/item-imports/{importId}/approve | OWNER/WORKER/PLATFORM | Approve and create |
| POST | /api/v1/business-admin/branches/{branchId}/item-imports/{importId}/cancel | OWNER/WORKER/PLATFORM | Cancel import |
