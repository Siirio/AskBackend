...
Runtime evidence on the disposable local schema returned the same exact product through both engines. Healthy retrieval reported `MEILISEARCH`; the outage drill reported `POSTGRESQL`, `fallback_used=true`, and the connection reason.
