...
LOCKED | Frontend-selected scope is immutable | Search runs within PRODUCT or SERVICE chosen by the customer; AI cannot change it | search request DTOs, StructuredSearchProcessor
...
LOCKED | Search scope is PRODUCT or SERVICE, no standalone business search | Business page opens from item/service context, not as search result type | search_document, SearchSession
LOCKED | Search creates no outreach or chat state | Search is catalog retrieval only; customer contact is an explicit separate action | search domain, chat domain
