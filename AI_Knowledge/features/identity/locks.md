...
LOCKED | Authenticated password and two-factor changes require owner- and purpose-bound email verification | Request endpoints stage intent only; public verification cannot consume security challenges; password confirmation preserves the current session and revokes the others | AuthController, AuthProcessor, IdentityServiceImpl, VerificationPurpose
LOCKED | Verification attempt limits are durable and 2FA applies to every role | Failed attempts commit independently of outer auth transactions; no platform-role password login may bypass enabled 2FA | IdentityServiceImpl.verifyCode, LoginProcessor
