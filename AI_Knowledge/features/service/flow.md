...
flowchart LR
    ...
    SO -. optional .-> BO[Branch-specific facts]
    SO --> O[Search outbox event]
    O --> S[Customer service search card]
    S -->|explicit action| C[Business-wide conversation]
    C --> D[Customer and business discuss details]
```

The customer sees service name, business, optional branch, and known price or schedule. Contact is explicit and opens/resumes the business-wide chat.
