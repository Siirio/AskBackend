...
LOCKED | Onboarding delivery data stays editable in BusinessProfile | The delivery-and-pickup screen is penultimate; coverage is one of NO_DELIVERY, SELECTED_CITIES, KAZAKHSTAN, WORLDWIDE, selected cities are required only for SELECTED_CITIES, and pickup is independent | Seller onboarding, BusinessProfile, business profile editor
LOCKED | Onboarding pickup branches persist atomically | Enabling pickup requires at least one map-selected branch; business, membership, profile, verification, and all drafted branches commit together and immediately appear in Organization | SellerOnboardingRequest, SellerOnboardingProcessor, Seller onboarding UI
